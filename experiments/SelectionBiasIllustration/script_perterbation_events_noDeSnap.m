...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Params.VAR.amp = 10; % amplitude of the input events through innovations
...
Params.Output.save_path = strcat(proj_path,'\Simulations\perturbation_events\saved_data\');
...
Params.BIC.momax = 15;
Params.BIC.tau = 1;
Params.BIC.mode = 'biased'; % "biased" or "debiased"
Params.BIC.morder = 4;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Params.DeSnap_inputs.maxStdRatio = 6.5;
Params.DeSnap_inputs.N_d = 100;
...
if Params.Options.generate_flag
[x] = simul_AR_kaidi_nonstat_innomean(Params.VAR.A, Params.VAR.SIG, Imp_long, Params.VAR.morder);
Imp_long(:,1:Params.VAR.morder)=[];
else
    load('C:\Users\skd\OneDrive\updated-desnap-with-causality\saved_data\presaved_long_signal_perturb_events.mat')
end
...
switch align_flag
    ...
    case 1
        Params.Options.Detection = 0;
        D = x(dim_align,locs);
        Params.Detection.ThresRatio = 2.5;
        d0 = mean(D)+Params.Detection.ThresRatio*std(D);
%         if dim_align==1 d0 = 15; end
%         if dim_align==2 d0=11.5; end
        loc_idx = find(x(dim_align,locs)>d0);
        Params.Detection.locs = locs(loc_idx);   
        [SnapAnalyOutput] = snapshot_detect_analysis_pipeline(x, {xf(1,:); xf(1,:)}, Params);
    case 2
        Params.Options.Detection = 1; 
        Params.Detection.ThresRatio = 4.6;
